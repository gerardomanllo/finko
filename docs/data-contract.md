...
|-----------------|-----------------------------------|
| **Dashboard** | `accountsStreamProvider`; `userProfileStreamProvider`; **`monthlyTotalsForMonthStreamProvider(dashboardYearMonthProvider)`** (MTD expense/budget math through profile today); **`netWorthSparklineSeriesProvider`** (30-day net worth ending profile today); **`dashboardMonthDailyExpenseSeriesProvider`** (**running total** expense through each calendar day in `dashboardYearMonth`, full month length); **`recentTransactionsStreamProvider`** (last 5 **≤ today**); **`dashboardUpcomingStripProvider`** ( **`upcomingTransactions`** + **`futureDatedLedgerTransactionsStreamProvider`** + recurring previews); pull-to-refresh invalidates streams + **`materializeDueUpcoming`** + **`reconcileDeferredLedgerForUser`**. Optional `forexRates` if UI shows rate context. |
...
| 2026-04-16 | **§5** Budgets row: **`userProfileStreamProvider`** for **`users/{uid}.budgets`** + **`monthlyTotalsForMonthStreamProvider`** for month actuals; **`categoriesStreamProvider`** for labels. |
